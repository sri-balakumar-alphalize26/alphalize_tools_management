...
class ModulePrivilege(models.Model):
    ...
    def _auto_apply_privileges(self):
        ...
        UserPrivilege = self.env['user.privilege.rental'].sudo()
        for rec in self:
    ...
    def _ensure_module_menus_visible(self):
        ...
        for rec in self:
            ...
            try:
                ...
                cr.execute("""
                    SELECT menu_id FROM menu_privilege_rental
                ...
                if missing:
                    for mid in missing:
                        cr.execute("""
                            INSERT INTO menu_privilege_rental
                ...
                cr.execute("""
                    UPDATE module_visibility_rental
                    SET is_visible = true,
                        write_uid = %s, write_date = NOW() AT TIME ZONE 'UTC'
    ...
    def action_add_module_to_role(self, user_id, module_id, role_id):
        ...
        """
        role = self.env['privilege.role.rental'].sudo().browse(role_id)
        if not role.exists():
    ...
    def create_module_menu_privileges(self, module_id, user_id):
        ...
        self.env.cr.execute("""
            SELECT menu_id, id, is_visible FROM menu_privilege_rental
            WHERE user_id = %s AND menu_id = ANY(%s)
        """, (user_id, list(menus.ids)))
        existing = {r[0]: {'id': r[1], 'is_visible': r[2]} for r in self.env.cr.fetchall()}

        MenuPriv = self.env['menu.privilege.rental'].sudo()
        created = 0
        updated = 0
        for menu in menus:
    ...
    def check_user_admin_status(self, user_id):
        ...
        checks = {
            ...
            'has_privilege_manager': 'tool_rental_privilege_manager.group_privilege_apps_manager',
        }
        result = {
            'is_admin': False,
            'bypass_group_names': [],
            ...
            try:
                ...
                try:
                    our_group = self.env.ref(
                        'tool_rental_privilege_manager.group_privilege_apps_manager',
    ...
    def get_user_primary_role(self, user_id):
        ...
        role = self.env['privilege.role.rental'].sudo().search([
            ('user_ids', 'in', [user_id]),
            ('active', '=', True),
        ], limit=1)
        if role:
    ...
    def action_apply_privileges(self):
        ...
        UserPrivilege = self.env['user.privilege.rental']
        for rec in self:
            for line in rec.line_ids:
                existing = UserPrivilege.search([
                    ('user_id', '=', rec.user_id.id),
...
class ModulePrivilegeLine(models.Model):
    _name = 'module.privilege.line.rental'
