...
def post_init_hook(env):
    ...
    """
    try:
        ModPriv = env['module.privilege.rental'].sudo()
        UserPriv = env['user.privilege.rental'].sudo()
        PrivRole = env['privilege.role.rental'].sudo()
    ...
    try:
        env['module.privilege.rental'].sudo().action_sync_all_to_user_privileges()
    except Exception:
        pass
    ...
    try:
        all_roles = env['privilege.role.rental'].sudo().search([('active', '=', True)])
