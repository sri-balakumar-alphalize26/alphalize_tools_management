...
class WhatsAppController(http.Controller):
    ...
    def qr_status(self, session_id, **kwargs):
        ...
        """
        from odoo.addons.whatsapp_neonize_server.models.whatsapp_session import (
            _wa_status, _wa_qr_image,
        )

        session = request.env['whatsapp.session'].browse(session_id)
        if not session.exists():
            _logger.warning("❌ Session %s not found", session_id)
            return {'status': 'error', 'message': 'Session not found'}

        # Prefer in-memory status; fall back to DB status
        mem_status = _wa_status.get(session_id) or session.status
        _logger.warning("🔵 QR status poll: session=%s | mem_status=%s | db_status=%s", 
                       session_id, mem_status, session.status)
        ...
        if mem_status == 'waiting_qr':
            ...
            if qr:
                _logger.warning("🔵 QR found in memory: %s bytes | First 50 chars: %s", 
                              len(qr), qr[:50] if isinstance(qr, str) else "bytes")
                # qr is now always a base64 string
                result['qr_image'] = qr if isinstance(qr, str) else qr.decode('utf-8')
                
            elif session.qr_image:
                # DB fallback — critical for multi-worker servers
                qr_data = session.qr_image
                _logger.warning("🔵 QR found in DB: type=%s | len=%s | First 50 chars: %s", 
                              type(qr_data).__name__, len(qr_data) if qr_data else 0,
                              str(qr_data)[:50] if qr_data else "empty")
                # session.qr_image is base64 string from Binary field
                result['qr_image'] = qr_data if isinstance(qr_data, str) else str(qr_data)
            else:
                _logger.warning("❌ NO QR IMAGE FOUND - Neither in memory nor DB!")
    ...
    @http.route(
        '/whatsapp/debug/<int:session_id>',
        type='http', auth='user', methods=['GET'],
    )
    def debug_qr(self, session_id, **kwargs):
        """Debug endpoint to check QR data in database and memory."""
        from odoo.addons.whatsapp_neonize_server.models.whatsapp_session import (
            _wa_qr_image, _wa_status, _wa_clients, _wa_threads,
        )
        ...
        debug_info = {
            ...
            'session_id': session_id,
            'session_status': session.status if session.exists() else None,
            'session_db_path': session.db_path if session.exists() else None,
            'db_path_exists': bool(session.db_path and os.path.exists(session.db_path)),
            'in_memory_status': _wa_status.get(session_id),
            'in_memory_qr_exists': session_id in _wa_qr_image,
            'in_memory_qr_length': len(_wa_qr_image.get(session_id, '')) if session_id in _wa_qr_image else 0,
            'client_exists': session_id in _wa_clients,
            'thread_exists': session_id in _wa_threads,
            'thread_alive': False,
            'db_qr_exists': bool(session.qr_image) if session.exists() else False,
            'db_qr_type': str(type(session.qr_image).__name__) if session.exists() and session.qr_image else None,
            'db_qr_length': len(session.qr_image) if session.exists() and session.qr_image else 0,
            'error_message': session.error_message if session.exists() else None,
        }
        if session_id in _wa_threads:
            debug_info['thread_alive'] = _wa_threads[session_id].is_alive()

        if session.exists() and session.qr_image:
            qr_sample = str(session.qr_image)[:100] if session.qr_image else ''
            debug_info['db_qr_sample'] = qr_sample

        _logger.warning("🟠 DEBUG QR INFO: %s", debug_info)
        return request.make_response(f"<pre>{str(debug_info)}</pre>")
