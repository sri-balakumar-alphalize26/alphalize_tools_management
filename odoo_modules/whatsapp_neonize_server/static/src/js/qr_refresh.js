    ...
    _waStartPolling() {
        ...
        this._waPollingId = setInterval(async () => {
            ...
            try {
                ...
                if (result) {
                    const currentQr = record.data.qr_image;
                    const hasNewQr = result.status === "waiting_qr" && result.qr_image;
                    ...
                    if (hasNewQr && !currentQr) {
                        // QR image is now available in the backend; refresh the form
                        await record.load();
                        return;
                    }
                }
