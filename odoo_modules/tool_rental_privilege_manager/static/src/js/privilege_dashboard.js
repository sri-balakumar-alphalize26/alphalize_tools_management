...
class PrivilegeDashboard extends Component {
    static template = "tool_rental_privilege_manager.PrivilegeDashboard";
    ...
    async onUpdatePrivileges() {
        ...
        try {
            ...
            const ORDER = [
                'privilege.role.rental', 'module.privilege.rental',
                'role.privilege.line.rental', 'module.privilege.line.rental',
                'menu.privilege.rental', 'special.menu.privilege.rental', 'module.visibility.rental',
                'role.menu.visibility.rental', 'role.module.visibility.rental',
            ];
            ...
            if (byModel['access_group'] && this.state.selectedUserId) {
                for (const { id: groupId, fields } of byModel['access_group']) {
                    await this.orm.call(
                        "module.privilege.rental", "toggle_user_group",
            ...
            if (byModel['module.privilege.line.rental']) {
                const mpIds = new Set();
                for (const mp of this.state.modules) {
                    for (const line of (mp.lines || [])) {
                        if (byModel['module.privilege.line.rental'].some(c => c.id === line.id)) {
                            mpIds.add(mp.id);
                        }
                    }
                }
                for (const mpId of mpIds) {
                    await this.orm.call("module.privilege.rental", "action_apply_privileges", [[mpId]]);
                }
            }

            // Also apply for module.privilege master toggle changes
            if (byModel['module.privilege.rental']) {
                for (const { id } of byModel['module.privilege.rental']) {
                    await this.orm.call("module.privilege.rental", "action_apply_privileges", [[id]]);
                }
            }

            // Sync Odoo groups when role privileges change (ensures dependent module access)
            if ((byModel['role.privilege.line.rental'] || byModel['privilege.role.rental']) && this.state.selectedRoleId) {
                await this.orm.call("privilege.role.rental", "action_sync_groups_now", [[this.state.selectedRoleId]]);
            }
    ...
    async _loadInstalledModules() {
        ...
        try {
            const moduleToMenuName = await this.orm.call("privilege.role.rental", "get_module_menu_labels", []);
    ...
    async _refreshModuleClassification(userId) {
        try {
            const classification = await this.orm.call(
                "module.privilege.rental", "classify_user_modules",
                [], { user_id: userId }
            );
    ...
    async _checkUserAdminStatus(userId) {
        try {
            const status = await this.orm.call(
                "module.privilege.rental", "check_user_admin_status",
                [], { user_id: userId }
            );
    ...
    async onCleanupAdminGroups() {
        ...
        try {
            const result = await this.orm.call(
                "module.privilege.rental", "cleanup_admin_groups",
    ...
    async _fetchUserRoles(userId) {
        try {
            const roles = await this.orm.searchRead("privilege.role.rental", [["user_ids", "in", [userId]], ["active", "=", true]], ["id", "name", "line_ids"], { order: "name" });
            for (const r of roles) {
                r.lines = r.line_ids && r.line_ids.length > 0
                    ? await this.orm.searchRead("role.privilege.line.rental", [["id", "in", r.line_ids]], ["id", "model_id", "model_name", "perm_read", "perm_create", "perm_write", "perm_cancel", "perm_unlink"], { order: "model_id" })
                    : [];
            }
            return roles;
        } catch (e) { return []; }
    ...
    async _fetchModulePrivileges(userId) {
        const mps = await this.orm.searchRead("module.privilege.rental", [["user_id", "=", userId], ["active", "=", true]],
        ...
        for (const mp of mps) {
            mp.lines = mp.line_ids && mp.line_ids.length > 0
                ? await this.orm.searchRead("module.privilege.line.rental", [["id", "in", mp.line_ids]], ["id", "model_id", "model_name", "perm_read", "perm_create", "perm_write", "perm_cancel", "perm_unlink"], { order: "model_id" })
                : [];
        }
        return mps;
    }
    ...
    async _fetchMenuPrivileges(userId) {
        return this.orm.searchRead("menu.privilege.rental", [["user_id", "=", userId], ["active", "=", true]], ["id", "menu_id", "menu_full_name", "is_visible", "source_module_id"], { order: "menu_full_name" });
    }
    async _fetchSpecialMenuPrivileges(userId) {
        try {
            return await this.orm.searchRead("special.menu.privilege.rental", [["user_id", "=", userId]], ["id", "menu_key", "is_visible"]);
        } catch(e) { return []; }
    }

    async _fetchModuleVisibility(userId) {
        return this.orm.searchRead("module.visibility.rental", [["user_id", "=", userId], ["active", "=", true]], ["id", "module_id", "module_shortdesc", "is_visible"], { order: "module_shortdesc" });
    }
    async _fetchAppFeatureVisibility(userId) {
        try {
            return await this.orm.searchRead(
                "app.feature.visibility.rental",
    ...
    async _loadAppFeatures() {
        try {
            this.state.allAppFeatures = await this.orm.searchRead(
                "app.feature.rental", [["active", "=", true]],
        ...
        try {
            ...
            try {
                depInfo = await this.orm.call(
                    "module.privilege.rental", "get_module_dependencies_info",
        ...
        try {
            ...
            for (const depId of extraModuleIds) {
                try {
                    ...
                    await this.orm.call("module.privilege.rental", "action_load_models", [[dmpId]]);
                    await this.orm.call("module.privilege.rental", "action_apply_privileges", [[dmpId]]);
                    await this.orm.call(
                        "module.privilege.rental", "create_module_menu_privileges", [],
            ...
            await this.orm.call("module.privilege.rental", "action_load_models", [[mpId]]);
            await this.orm.call("module.privilege.rental", "action_apply_privileges", [[mpId]]);
        ...
        for (const mp of modules) {
            ...
            try {
                const groups = await this.orm.call(
                    "module.privilege.rental", "get_module_access_groups",
                    [], { module_id: moduleId, user_id: userId }
                );
    ...
    async onGrantAllGroups(mpId) {
        ...
        try {
            const count = await this.orm.call(
                "module.privilege.rental", "grant_all_module_groups",
    ...
    onMasterToggle(mpId, field, cur) {
        ...
        const newVal = !cur;
        mp[field] = newVal;
        this._addPending('module.privilege.rental', mpId, field, newVal);
        ...
        for (const line of (mp.lines || [])) {
            ...
            this._addPending('module.privilege.line.rental', line.id, lineField, newVal);
        }
    }
    ...
    onLineToggle(lineId, field, cur) {
        ...
        for (const mp of this.state.modules) {
            ...
            if (line) {
                line[field] = !cur;
                this._addPending('module.privilege.line.rental', lineId, field, !cur);
                break;
            }
        }
    }
    ...
    onGrantAll(mpId) {
        ...
        for (const field of masterFields) {
            mp[field] = true;
            this._addPending('module.privilege.rental', mpId, field, true);
        }
        for (const line of (mp.lines || [])) {
            for (const field of lineFields) {
                line[field] = true;
                this._addPending('module.privilege.line.rental', line.id, field, true);
            }
        }
    }
    ...
    onRevokeAll(mpId) {
        ...
        this._addPending('module.privilege.rental', mpId, 'master_read', true);
        this._addPending('module.privilege.rental', mpId, 'master_create', false);
        this._addPending('module.privilege.rental', mpId, 'master_write', false);
        this._addPending('module.privilege.rental', mpId, 'master_cancel', false);
        this._addPending('module.privilege.rental', mpId, 'master_unlink', false);
        for (const line of (mp.lines || [])) {
            line.perm_read = true;
            ...
            this._addPending('module.privilege.line.rental', line.id, 'perm_read', true);
            this._addPending('module.privilege.line.rental', line.id, 'perm_create', false);
            this._addPending('module.privilege.line.rental', line.id, 'perm_write', false);
            this._addPending('module.privilege.line.rental', line.id, 'perm_cancel', false);
            this._addPending('module.privilege.line.rental', line.id, 'perm_unlink', false);
        }
    }
    ...
    async onFullPermission() {
        ...
        try {
            ...
            for (const mod of toAdd) {
                const result = await this.orm.create("module.privilege.rental", [{ user_id: userId, module_id: mod.id }]);
                const mpId = Array.isArray(result) ? result[0] : result;
                await this.orm.call("module.privilege.rental", "action_load_models", [[mpId]]);
                await this.orm.call("module.privilege.rental", "action_apply_privileges", [[mpId]]);
                await this.orm.call("module.privilege.rental", "create_module_menu_privileges", [], { module_id: mod.id, user_id: userId });
            }
            ...
            for (const mp of this.state.modules) {
                ...
                await this.orm.write("module.privilege.rental", [mp.id], masterVals);
                for (const line of (mp.lines || [])) {
                    const lineVals = {};
                    for (const f of lineFields) { line[f] = true; lineVals[f] = true; }
                    await this.orm.write("module.privilege.line.rental", [line.id], lineVals);
                }
            }
            ...
            for (const mp of this.state.menuPrivileges) {
                if (!mp.is_visible) {
                    mp.is_visible = true;
                    await this.orm.write("menu.privilege.rental", [mp.id], { is_visible: true });
                }
            }
            ...
            for (const sp of this.state.specialMenuPrivileges) {
                if (!sp.is_visible) {
                    sp.is_visible = true;
                    await this.orm.write("special.menu.privilege.rental", [sp.id], { is_visible: true });
                }
            }
            ...
            for (const mv of this.state.moduleVisibility) {
                if (!mv.is_visible) {
                    mv.is_visible = true;
                    await this.orm.write("module.visibility.rental", [mv.id], { is_visible: true });
                }
            }
    ...
    async onResetAllPrivileges() {
        ...
        try {
            ...
            const allMenuPrivs = await this.orm.searchRead("menu.privilege.rental", [["user_id", "=", userId]], ["id"]);
            if (allMenuPrivs.length > 0) {
                await this.orm.unlink("menu.privilege.rental", allMenuPrivs.map(m => m.id));
            }
            // Delete all special menu privileges
            if (this.state.specialMenuPrivileges.length > 0) {
                await this.orm.unlink("special.menu.privilege.rental", this.state.specialMenuPrivileges.map(s => s.id));
            }
            // Delete all module visibility
            if (this.state.moduleVisibility.length > 0) {
                await this.orm.unlink("module.visibility.rental", this.state.moduleVisibility.map(m => m.id));
            }
            // Delete all module privileges and their user.privilege records
            if (this.state.modules.length > 0) {
                const ups = await this.orm.searchRead("user.privilege.rental", [["user_id", "=", userId], ["source_module_id", "!=", false]], ["id"]);
                if (ups.length > 0) await this.orm.unlink("user.privilege.rental", ups.map(p => p.id));
                await this.orm.unlink("module.privilege.rental", this.state.modules.map(m => m.id));
            }
            this._clearPending();
            // Refresh all data
            const [mps, menuPrivs, specialMenuPrivs, modVis] = await Promise.all([
    ...
    async onRemoveModule(mpId) {
        try {
            const mp = this.state.modules.find(m => m.id === mpId);
            if (mp) {
                const ups = await this.orm.searchRead("user.privilege.rental", [["user_id", "=", this.state.selectedUserId], ["source_module_id", "=", mp.module_id[0]]], ["id"]);
                if (ups.length > 0) await this.orm.unlink("user.privilege.rental", ups.map(p => p.id));
                // Also remove menu_privilege records created for this module
                const menuPrivs = await this.orm.searchRead("menu.privilege.rental", [["user_id", "=", this.state.selectedUserId], ["source_module_id", "=", mp.module_id[0]]], ["id"]);
                if (menuPrivs.length > 0) await this.orm.unlink("menu.privilege.rental", menuPrivs.map(p => p.id));
            }
            await this.orm.unlink("module.privilege.rental", [mpId]);
    ...
    async onCleanupModulePrivileges() {
        try {
            const ups = await this.orm.searchRead("user.privilege.rental", [["user_id", "=", this.state.selectedUserId], ["source_module_id", "!=", false]], ["id"]);
            if (ups.length > 0) await this.orm.unlink("user.privilege.rental", ups.map(p => p.id));
            const mps = await this.orm.searchRead("module.privilege.rental", [["user_id", "=", this.state.selectedUserId]], ["id"]);
            if (mps.length > 0) await this.orm.unlink("module.privilege.rental", mps.map(p => p.id));
            this._clearPending();
            this.state.modules = await this._fetchModulePrivileges(this.state.selectedUserId);
    ...
    async onPickMenu(menuId) {
        ...
        try {
            if (typeof menuId === "string" && menuId.startsWith("__special_")) {
                ...
                if (sm) {
                    await this.orm.create("special.menu.privilege.rental", [{ user_id: this.state.selectedUserId, menu_key: sm.key, is_visible: false }]);
                ...
                await this.orm.create("menu.privilege.rental", [{ user_id: this.state.selectedUserId, menu_id: menuId, is_visible: false }]);
                this.state.menuPrivileges = await this._fetchMenuPrivileges(this.state.selectedUserId);
    ...
    onSpecialMenuVisToggle(spId, cur) {
        ...
        if (sp) {
            sp.is_visible = !cur;
            this._addPending('special.menu.privilege.rental', spId, 'is_visible', !cur);
        }
    }

    async onRemoveSpecialMenuPriv(spId) {
        try {
            await this.orm.unlink("special.menu.privilege.rental", [spId]);
    ...
    onMenuVisToggle(mpId, cur) {
        ...
        if (mp) {
            mp.is_visible = !cur;
            this._addPending('menu.privilege.rental', mpId, 'is_visible', !cur);
        }
    }

    async onRemoveMenuPriv(mpId) {
        try {
            await this.orm.unlink("menu.privilege.rental", [mpId]);
    ...
    async onPickModuleVis(moduleId) {
        this.state.showAddModuleVis = false;
        try {
            await this.orm.create("module.visibility.rental", [{ user_id: this.state.selectedUserId, module_id: moduleId, is_visible: false }]);
    ...
    onModuleVisToggle(mvId, cur) {
        ...
        if (mv) {
            mv.is_visible = !cur;
            this._addPending('module.visibility.rental', mvId, 'is_visible', !cur);
        }
    }

    async onRemoveModuleVis(mvId) {
        try {
            await this.orm.unlink("module.visibility.rental", [mvId]);
    ...
    async onPickAppFeature(featureId) {
        this.state.showAddAppFeature = false;
        try {
            await this.orm.create("app.feature.visibility.rental", [{
                user_id: this.state.selectedUserId,
    ...
    async onRemoveAppFeature(afvId) {
        try {
            await this.orm.unlink("app.feature.visibility.rental", [afvId]);
            this.state.appFeatureVisibility = await this._fetchAppFeatureVisibility(this.state.selectedUserId);
        } catch (e) { this.notification.add("Error: " + (e.message || e), { type: "danger" }); }
    }
    ...
    async _loadRoleLines(roleId) {
        ...
        try {
            const roleData = await this.orm.read("privilege.role.rental", [roleId], [
            ...
            if (roleData && roleData.length > 0) {
                ...
                this.state.roleLines = rd.line_ids && rd.line_ids.length > 0
                    ? await this.orm.searchRead("role.privilege.line.rental", [["id", "in", rd.line_ids]], ["id", "model_id", "model_name", "source_module_id", "source_module_shortdesc", "perm_read", "perm_create", "perm_write", "perm_cancel", "perm_unlink"], { order: "source_module_id, model_id" })
                    : [];
                ...
                this.state.roleMenuVisibility = rd.menu_visibility_ids && rd.menu_visibility_ids.length > 0
                    ? await this.orm.searchRead("role.menu.visibility.rental", [["id", "in", rd.menu_visibility_ids]], ["id", "menu_id", "menu_full_name", "is_visible"], { order: "menu_full_name" })
                    : [];

                this.state.roleModuleVisibility = rd.module_visibility_ids && rd.module_visibility_ids.length > 0
                    ? await this.orm.searchRead("role.module.visibility.rental", [["id", "in", rd.module_visibility_ids]], ["id", "module_id", "module_shortdesc", "is_visible"], { order: "module_shortdesc" })
                    : [];
            }
    ...
    onRoleMasterToggle(field, cur) {
        ...
        this._addPending('privilege.role.rental', roleId, field, newVal);

        // Cascade to all role lines
        const lineField = field.replace('master_', 'perm_');
        for (const line of this.state.roleLines) {
            line[lineField] = newVal;
            this._addPending('role.privilege.line.rental', line.id, lineField, newVal);
        }
    }
    ...
    onRoleLineToggle(lineId, field, cur) {
        ...
        if (line) {
            line[field] = !cur;
            this._addPending('role.privilege.line.rental', lineId, field, !cur);
        }
    }
    ...
    onRoleModuleGrantAll(moduleId) {
        ...
        for (const line of mod.lines) {
            for (const field of lineFields) {
                line[field] = true;
                this._addPending('role.privilege.line.rental', line.id, field, true);
            }
        }
    }
    ...
    onRoleModuleRevokeAll(moduleId) {
        ...
        for (const line of mod.lines) {
            ...
            line.perm_cancel = false;
            line.perm_unlink = false;
            this._addPending('role.privilege.line.rental', line.id, 'perm_read', true);
            this._addPending('role.privilege.line.rental', line.id, 'perm_create', false);
            this._addPending('role.privilege.line.rental', line.id, 'perm_write', false);
            this._addPending('role.privilege.line.rental', line.id, 'perm_cancel', false);
            this._addPending('role.privilege.line.rental', line.id, 'perm_unlink', false);
        }
    }
    ...
    async onRemoveRoleModulePriv(moduleId) {
        ...
        try {
            await this.orm.call("privilege.role.rental", "action_remove_module_from_role", [[this.state.selectedRoleId], moduleId]);
            await this._loadRoleLines(this.state.selectedRoleId);
    ...
    async onPickRoleModulePriv(moduleId) {
        ...
        try {
            ...
            const result = await this.orm.call("privilege.role.rental", "action_add_module_models", [[this.state.selectedRoleId], moduleId]);
            // 2. Add module menus to role menu visibility (like user flow's create_module_menu_privileges)
            const menusCreated = await this.orm.call("privilege.role.rental", "action_add_module_menus", [[this.state.selectedRoleId], moduleId]);
    ...
    onRoleGrantAll() {
        ...
        for (const field of masterFields) {
            ...
            this._addPending('privilege.role.rental', roleId, field, true);
        }
        for (const line of this.state.roleLines) {
            for (const field of lineFields) {
                line[field] = true;
    ...
    onRoleRevokeAll() {
        ...
        this._addPending('privilege.role.rental', roleId, 'master_read', true);
        this._addPending('privilege.role.rental', roleId, 'master_create', false);
        this._addPending('privilege.role.rental', roleId, 'master_write', false);
        this._addPending('privilege.role.rental', roleId, 'master_cancel', false);
        this._addPending('privilege.role.rental', roleId, 'master_unlink', false);
        for (const line of this.state.roleLines) {
            line.perm_read = true;
            line.perm_create = false;
            line.perm_write = false;
    ...
    async onResetRolePrivileges() {
        ...
        try {
            ...
            if (this.state.roleMenuVisibility.length > 0) {
                await this.orm.unlink("role.menu.visibility.rental", this.state.roleMenuVisibility.map(m => m.id));
            }
            // Delete all role module visibility
            if (this.state.roleModuleVisibility.length > 0) {
                await this.orm.unlink("role.module.visibility.rental", this.state.roleModuleVisibility.map(m => m.id));
            }
            // Delete all role privilege lines (model privileges)
            if (this.state.roleLines.length > 0) {
                await this.orm.unlink("role.privilege.line.rental", this.state.roleLines.map(l => l.id));
            }
            this._clearPending();
            await this._loadRoleLines(roleId);
    ...
    async _loadRoles() {
        const roles = await this.orm.searchRead("privilege.role.rental", [["active", "=", true]], ["id", "name", "user_count", "user_ids"], { order: "name" });
    ...
    async onPickRoleMenu(menuId) {
        this.state.showAddRoleMenu = false;
        try {
            await this.orm.create("role.menu.visibility.rental", [{ role_id: this.state.selectedRoleId, menu_id: menuId, is_visible: false }]);
            await this._loadRoleLines(this.state.selectedRoleId);
    ...
    onRoleMenuVisToggle(id, cur) {
        ...
        if (rm) {
            rm.is_visible = !cur;
            this._addPending('role.menu.visibility.rental', id, 'is_visible', !cur);
        }
    }

    async onRemoveRoleMenu(id) {
        try {
            await this.orm.unlink("role.menu.visibility.rental", [id]);
    ...
    async onPickRoleModule(moduleId) {
        this.state.showAddRoleModule = false;
        try {
            await this.orm.create("role.module.visibility.rental", [{ role_id: this.state.selectedRoleId, module_id: moduleId, is_visible: false }]);
            await this._loadRoleLines(this.state.selectedRoleId);
    ...
    onRoleModuleVisToggle(id, cur) {
        ...
        if (rmod) {
            rmod.is_visible = !cur;
            this._addPending('role.module.visibility.rental', id, 'is_visible', !cur);
        }
    }

    async onRemoveRoleModule(id) {
        try {
            await this.orm.unlink("role.module.visibility.rental", [id]);
...
registry.category("actions").add("privilege_dashboard_rental", PrivilegeDashboard);
