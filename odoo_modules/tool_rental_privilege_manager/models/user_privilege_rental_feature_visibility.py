...
class AppFeatureVisibility(models.Model):
    ...
    _name = 'app.feature.visibility.rental'
    ...
    def get_hidden_features_for_user(self, user_id=None):
        ...
        role_keys = self.env['privilege.role.rental'].sudo().get_hidden_features_by_role(user_id)
    ...
    def hide_all_features_for_user(self, user_id):
        if not user_id:
            return 0
        uid = int(user_id)
        features = self.env['app.feature.rental'].sudo().search([])
    ...
    def get_privilege_stats_for_user(self, user_id):
        ...
        roles = self.env['privilege.role.rental'].sudo().search([
            ('user_ids', 'in', [uid]),
            ('active', '=', True),
        ])

        user_hidden_modules = set(self.env['module.visibility.rental'].sudo().search([
            ('user_id', '=', uid),
            ('is_visible', '=', False),
            ('active', '=', True),
        ...
        user_hidden_menus = set(self.env['menu.privilege.rental'].sudo().search([
            ('user_id', '=', uid),
            ('is_visible', '=', False),
        ]).mapped('menu_id').ids)
        ...
        return {
            'groups': len(roles),
            'modules': self.env['module.privilege.rental'].sudo().search_count([
                ('user_id', '=', uid),
                ('active', '=', True),
            ]),
    ...
    def list_user_modules_admin(self, user_id):
        ...
        if not user_id:
            return []
        rows = self.env['module.privilege.rental'].sudo().search([
            ('user_id', '=', int(user_id)),
            ('active', '=', True),
    ...
    def list_installable_modules_admin(self, user_id, search_text=''):
        if not user_id:
            return []
        used_module_ids = self.env['module.privilege.rental'].sudo().search([
            ('user_id', '=', int(user_id)),
            ('active', '=', True),
        ]).mapped('module_id').ids
    ...
    def set_module_master_perm_admin(self, mp_id, field, value):
        ...
        mp = self.env['module.privilege.rental'].sudo().browse(int(mp_id))
        if not mp.exists():
    ...
    def add_module_admin(self, user_id, module_id):
        ...
        MP = self.env['module.privilege.rental'].sudo()
    ...
    def remove_module_admin(self, mp_id):
        ...
        if not MP.exists():
            return False
        derived_user_privs = self.env['user.privilege.rental'].sudo().search(
            [('source_module_id', '=', MP.id)])
        if derived_user_privs:
            derived_user_privs.unlink()
        if 'source_module_id' in self.env['menu.privilege.rental']._fields:
            derived_menu_privs = self.env['menu.privilege.rental'].sudo().search(
                [('source_module_id', '=', MP.id)])
