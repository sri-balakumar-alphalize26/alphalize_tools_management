...
class PrivilegeBulkCreateWizard(models.TransientModel):
    _name = 'privilege.bulk.create.wizard.rental'
...
class MenuPrivilegeBulkWizard(models.TransientModel):
    _name = 'menu.privilege.bulk.wizard.rental'
    _description = 'Bulk Create Menu Privileges (Apps)'

    user_ids = fields.Many2many(
        'res.users',
        'menu_privilege_bulk_wizard_rental_user_rel',
        'wizard_id', 'user_id',
        string='Users',
        domain=[('share', '=', False)],
        required=True,
    ...
    menu_ids = fields.Many2many(
        'ir.ui.menu',
        'menu_privilege_bulk_wizard_rental_menu_rel',
        'wizard_id', 'menu_id',
        string='Menus',
        required=True,
    )
    ...
    def action_create_menu_privileges(self):
        self.ensure_one()
        MenuPrivilege = self.env['menu.privilege.rental']
        for user in self.user_ids:
...
class ModuleVisibilityBulkWizard(models.TransientModel):
    _name = 'module.visibility.bulk.wizard.rental'
    _description = 'Bulk Set Module (App) Visibility (Apps)'

    user_ids = fields.Many2many(
        'res.users',
        'module_visibility_bulk_wizard_rental_user_rel',
        'wizard_id', 'user_id',
        string='Users',
        domain=[("share", "=", False)],
        required=True,
    ...
    module_ids = fields.Many2many(
        'ir.module.module',
        'module_visibility_bulk_wizard_rental_mod_rel',
        'wizard_id', 'module_id',
        string='Modules (Apps)',
    ...
    def action_apply(self):
        self.ensure_one()
        ModuleVis = self.env['module.visibility.rental']
        for user in self.user_ids:
