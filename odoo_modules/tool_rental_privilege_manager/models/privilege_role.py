...
class PrivilegeRole(models.Model):
    ...
    user_ids = fields.Many2many(
        'res.users',
        'privilege_role_rental_users_rel',
        'role_id', 'user_id',
        string='Users',
    )
    ...
    odoo_group_ids = fields.Many2many(
        'res.groups',
        'privilege_role_rental_odoo_groups_rel',
        'role_id', 'group_id',
    ...
    company_ids = fields.Many2many(
        'res.company',
        'privilege_role_rental_company_rel',
        'role_id', 'company_id',
        string='Companies',
        default=lambda self: self.env.company,
    ...
    line_ids = fields.One2many(
        'role.privilege.line.rental', 'role_id',
        string='Model Privileges',
        copy=True,
    )

    menu_visibility_ids = fields.One2many(
        'role.menu.visibility.rental', 'role_id',
    ...
    module_visibility_ids = fields.One2many(
        'role.module.visibility.rental', 'role_id',
    ...
    app_feature_visibility_ids = fields.One2many(
        'role.app.feature.visibility.rental', 'role_id',
    ...
    add_model_ids = fields.Many2many(
        'ir.model',
        'privilege_role_rental_add_model_rel',
        'role_id', 'model_id',
        string='Add Models',
    ...
    def _apply_visibility_to_users(self, user_ids):
        ...
        MenuPrivilege = self.env['menu.privilege.rental'].sudo()
        ModuleVis = self.env['module.visibility.rental'].sudo()
            ...
            for modv in rec.module_visibility_ids.filtered(lambda x: not x.is_visible):
                ...
                for uid in user_ids:
                    if modv.module_name not in other_hidden_mods:
                        self.env['module.visibility.rental'].sudo()._sync_menu_privileges(
    ...
    def action_open_add_models_wizard(self):
        ...
        return {
            'name': 'Add Models',
            'type': 'ir.actions.act_window',
            'res_model': 'role.add.models.wizard.rental',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_add_module_models(self, module_id):
        ...
        created = self.env['role.privilege.line.rental'].sudo().create(new_lines) if new_lines else self.env['role.privilege.line.rental']
    ...
    def action_add_module_menus(self, module_id):
        ...
        RoleMenuVis = self.env['role.menu.visibility.rental'].sudo()
        created = 0
        for menu in menus:
    ...
    def get_role_privileges(self, model_name, user_id):
        ...
        lines = self.env['role.privilege.line.rental'].sudo().search([
...
class RolePrivilegeLine(models.Model):
    ...
    _name = 'role.privilege.line.rental'
    _description = 'Group Privilege Line (Apps)'
    _order = 'model_id'

    role_id = fields.Many2one('privilege.role.rental', required=True, ondelete='cascade')
...
class RoleMenuVisibility(models.Model):
    ...
    _name = 'role.menu.visibility.rental'
...
class RoleModuleVisibility(models.Model):
    ...
    _name = 'role.module.visibility.rental'
    ...
    def _push_to_role_users(self, is_visible):
        ...
        ModVis = self.env['module.visibility.rental'].sudo()
        for user in self.role_id.user_ids:
...
class RoleAppFeatureVisibility(models.Model):
    ...
    _name = 'role.app.feature.visibility.rental'
